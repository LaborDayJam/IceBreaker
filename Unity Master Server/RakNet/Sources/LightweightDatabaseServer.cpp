...
PluginReceiveResult LightweightDatabaseServer::OnReceive(Packet *packet)
	...
	switch (packet->data[0])
		...
		case ID_DATABASE_QUERY_REQUEST:
			info_log("Query request from %s\n", packet->systemAddress.ToString());
			lastMinQueryCount++;
			OnQueryRequest(packet);
			return RR_STOP_PROCESSING_AND_DEALLOCATE;
		case ID_DATABASE_UPDATE_ROW:
			info_log("Update row request from %s\n", packet->systemAddress.ToString());
			lastMinUpdateCount++;
			OnUpdateRow(packet);
			return RR_STOP_PROCESSING_AND_DEALLOCATE;
...
void LightweightDatabaseServer::OnClosedConnection(SystemAddress systemAddress, RakNetGUID rakNetGUID, PI2_LostConnectionReason lostConnectionReason )
{
	...
	info_log("Connection with %s closed\n", systemAddress.ToString());
		...
		for (rowIndex=0; rowIndex < (unsigned)cur->size; rowIndex++)
		{
			...
			if (strcmp(cur->data[rowIndex]->cells[IPINDEX]->c,packet->systemAddress.ToString(false))==0)
			{
				...
				if (addressMap.count(tempID) > 0)
				{
					...
					int counter = 0;
					debug_log("Injecting %d internal IPs: ", ipCount);
					...
					internalIPString << "\n";
					debug_log(internalIPString.str().c_str());
				...
				{
					error_log("Server (with NAT enabled) and client IPs, %s, match but server internal IP was not found during lookup\n", packet->systemAddress.ToString(false));
				}
			}
		}
		cur=cur->next;
...
void LightweightDatabaseServer::OnUpdateRow(Packet *packet)
{
	...
	if (databaseTable==0)
		{
		error_log("Table not found and failed to dynamically create table\n");
		return;
		}
	if (databaseTable->allowRemoteUpdate==false)
		{
		error_log("Remote updates not allowed by table design\n");
		return;
		}
	...
	for (i=0; i < numCellUpdates; i++)
		...
		if (cellUpdates[i].Deserialize(&inBitstream)==false)
			{
			error_log("LightweightDatabaseServer::OnUpdateRow cellUpdates deserialize failed i=%i numCellUpdates=%i\n",i,numCellUpdates);
			return;
			}
		}
	...
	if ((RowUpdateMode)updateMode==RUM_UPDATE_EXISTING_ROW)
	{
		if (hasRowId==false)
		{
			...
			if (row==0)
			{
				error_log("Attempting to update existing row without supplying row ID");
				return;
			}
		}
		else
		{
			...
			{
				if (row==0)
					error_log("LightweightDatabaseServer::OnUpdateRow row = databaseTable->table.GetRowByID(rowId); row==0\n");
				else
					error_log("Attempting to update someone elses row while in RUM_UPDATE_EXISTING_ROW\n");
		...
		if (row==0)
		{
			if (hasRowId) warn_log("Failed to find given row ID %u from %s\n",rowId, packet->systemAddress.ToString());
			...
			info_log("Sent row ID %u in %s to host at %s\n", rowId, databaseTable->tableName, packet->systemAddress.ToString());
			if (row==0)
				{
				error_log("Row error while performing RUM_UPDATE_OR_ADD_ROW, row update failed\n");
				return;
				}
			}
		else
			...
			if (databaseTable->onlyUpdateOwnRows && RowHasIP(row, packet->systemAddress, databaseTable->SystemAddressColumnIndex)==false)
				...
				warn_log("Attempting to update someone elses row, while in RUM_UPDATE_OR_ADD_ROW. caller=%s owner=%s. Creating new row.\n",
					str1, str2);
				
				warn_log("Attempting to update someone elses row, while in RUM_UPDATE_OR_ADD_ROW. Creating new row.\n");
				row=AddRow(databaseTable, packet->systemAddress, packet->guid, false, rowId);
				RakNet::BitStream stream;
				stream.Write((unsigned char)ID_DATABASE_ROWID);
				stream.Write(rowId);
				SendUnified(&stream, HIGH_PRIORITY, RELIABLE_ORDERED, 0, packet->systemAddress, false);
				warn_log("Sent row ID %u to host at %s\n",rowId, packet->systemAddress.ToString());
		...
		row=AddRow(databaseTable, packet->systemAddress, packet->guid, hasRowId, rowId);
		info_log("Created new row %u in %s for %s\n", rowId, databaseTable->tableName, packet->systemAddress.ToString());
		...
		info_log("Sent row ID %u to client %s\n",rowId, packet->systemAddress.ToString());
		
		if (row==0)
			{
			error_log("ERROR: LightweightDatabaseServer::OnUpdateRow updateMode==RUM_ADD_NEW_ROW; row==0\n");
			return;
			}
		}
		...
		if (columnIndex!=(unsigned)-1 &&
			...
			else if (cellUpdates[i].columnType==databaseTable->table.GetColumnType(columnIndex))
			{
				...
				else
				{
					...
					debug_log("Updating, inserting %s at column %d\n",cellUpdates[i].cellValue.c, columnIndex);
				}
			}
		}
	}
...
void LightweightDatabaseServer::OnRemoveRow(Packet *packet)
{
	...
	if (databaseTable==0)
	{
		info_log("Failed to remove row for %s because table lookup failed\n",packet->systemAddress.ToString());
		return;
	}
	if (databaseTable->allowRemoteRemove==false)
	{
		info_log("Remote row removal on %s forbidden, from %s\n", databaseTable->tableName, packet->systemAddress.ToString());
		return;
	}
	unsigned rowId;
	inBitstream.Read(rowId);
	...
	info_log("Removing row %u by request\n",rowId);
	debug_log("Current address map count is %d\n", addressMap.size());
}
...
bool LightweightDatabaseServer::CheckVersion(Packet *packet, RakNet::BitStream &inBitstream)
{
	...
	else if (memcmp(peerVersion, version009, sizeof(peerVersion))==0)
	{
		...
		{
			warn_log("Wrong version number but accepting connection\n");
			return true;
		}
		else
		{
			...
			info_log("Old version connected, %d.%d.%d, redirecting master server to %s, facilitator to %s\n", peerVersion[0], peerVersion[1], peerVersion[2], oldMasterServer.ToString(), oldFac);
	...
	{
		error_log("Incorrect master server client version, %d.%d.%d\n", peerVersion[0], peerVersion[1], peerVersion[2]);
...
LightweightDatabaseServer::DatabaseTable* LightweightDatabaseServer::CreateDefaultTable(std::string tableName)
{
	if (tableName.empty())
	{
		error_log("Error: Empty table name given during table creation. Aborting.\n");
		return 0;
	}
	...
	{
		error_log("Table %s creation failed.  Possibly already exists.\n", tableName.c_str());
		return 0;
	}
}
...
LightweightDatabaseServer::DatabaseTable * LightweightDatabaseServer::DeserializeClientHeader(RakNet::BitStream *inBitstream, Packet *packet, int mode, bool query)
	...
	{
		if (query)
			error_log("No table name defined, during query/removal.\n");
		else
			error_log("No table name defined.\n");
		error_log("Malformed packet from %s, packet size %d bits.\n", packet->systemAddress.ToString(), packet->bitSize);
		return 0;
	}
	DatabaseTable *databaseTable;
	...
	if (database.Has(tableName))
	{
		info_log("Fetching existing table %s\n", tableName);
		databaseTable = database.Get(tableName);
	}
	else
	{
		...
		if (query)
		{
			info_log("Table %s not found during query or row removal from %s\n", tableName, packet->systemAddress.ToString());
			return 0;
		}
		...
		if (databaseTable == 0)
		{
			error_log("No table selected. Cannot proceed.\n");
	...
	{
		warn_log("Incorrect table returned during table lookup, requested %s, got %s\n", tableName, databaseTable->tableName);
		if (query)
			return 0;
		databaseTable = CreateDefaultTable(tableName);
	}
	...
	if (databaseTable == 0)
	{
		error_log("Critical error: No database table selected just before being accessed. Something went wrong.\n");
		return 0;
	}
	...
	if (hasPassword)
		...
		if (stringCompressor->DecodeString(password, _SIMPLE_DATABASE_PASSWORD_LENGTH, inBitstream)==false)
			error_log("Error: Table password info not found\n");
			return 0;
			if (databaseTable->queryPassword[0] && strcmp(password, dbPass)!=0)
			{
				...
				error_log("Incorrect table password given, banning client\n");
				return 0;
			}
		}
	else if (dbPass[0])
		...
		outBitstream.Write((MessageID)ID_DATABASE_INCORRECT_PASSWORD);
		rakPeerInterface->Send(&outBitstream, HIGH_PRIORITY, RELIABLE_ORDERED, 0, packet->systemAddress, false);
		error_log("Error: Empty table password\n");
		return 0;
		}
...
DataStructures::Table::Row * LightweightDatabaseServer::AddRow(LightweightDatabaseServer::DatabaseTable *databaseTable, SystemAddress systemAddress, RakNetGUID guid, bool hasRowId, unsigned rowId)
	...
	{
		error_log("This system already has a row\n");
	...
	if (databaseTable->autogenerateRowIDs==false)
	{
		...
		{
			error_log("Error: Row ID error while creating row\n");
			return 0;
		}
	}
	else
...
void LightweightDatabaseServer::RemoveRowsFromIP(SystemAddress systemAddress)
{
	...
	info_log("Removing rows for IP %s\n",systemAddress.ToString());
	DatabaseTable *databaseTable;
	DataStructures::List<unsigned> removeList;
	DataStructures::Table::Row* row;
	DataStructures::Page<unsigned, DataStructures::Table::Row*, _TABLE_BPLUS_TREE_ORDER> *cur;
	...
	for (i=0; i < database.Size(); i++)
	{
		...
		for (j=0; j < removeList.Size(); j++)
		{
			...
			databaseTable->table.RemoveRow(removeList[j]);
			info_log("Force removing row %u\n",removeList[j]);
			debug_log("Current address map count %d\n", addressMap.size());
		}
		removeList.Clear(true, __FILE__,__LINE__);
		
		if (databaseTable->table.GetRowCount() == 0)
		{
			info_log("Deleting unused table for %s\n", databaseTable->tableName);
